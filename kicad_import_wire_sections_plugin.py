...
def draw_sections(board: "pcbnew.BOARD",
                  ...
                  fixed_layer_name: Optional[str] = None) -> Tuple[int, Dict[str, int]]:
    """
    mode: 'single' or 'zmap'
    ...
    if mode == "single":
        ...
        return created, per_layer_counts
...
def save_copy_in_folder(board: "pcbnew.BOARD", folder: str):
...
class ImportWireSectionsPlugin(pcbnew.ActionPlugin):
    def defaults(self):
        ...
        self.description = "Read Adress.txt → find Wire_Sections.txt → draw tracks per Section; map Z to layers and set copper layer count automatically."
    ...
    def Run(self):
        ...
        adress_path = ask_file_open(parent, "Choose Adress.txt (one folder per line)", "Text files (*.txt)|*.txt|All files|*.*")
        if not adress_path:
            return

        # Gather all Wire_Sections.txt
        try:
            folders = read_adress_file(adress_path)
        except Exception as e:
            wx.MessageBox(f"Failed to read Adress.txt:\n{e}", "Import Wire Sections", wx.OK | wx.ICON_ERROR)
            return

        if not folders:
            wx.MessageBox("Adress.txt contains no valid folders.", "Import Wire Sections", wx.OK | wx.ICON_WARNING)
            return
        ...
        for txt_path in txt_files:
            ...
            segs, per_layer = draw_sections(board, sections, mode, width_mm, fixed_layer)
            total_segments += segs
            ...
            if save_copies:
                saved = save_copy_in_folder(board, folder)
...
def read_adress_file(adress_path: str) -> List[str]:
    folders: List[str] = []
    with open(adress_path, "r", encoding="utf-8", errors="ignore") as f:
        for raw in f:
            line = raw.strip()
            if not line:
                continue
            if os.path.isdir(line):
                ...
                print(f"[Adress] Warning: not a folder → {line}")
    return folders
